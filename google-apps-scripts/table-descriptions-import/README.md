...
This script works with the Django tables audit spreadsheet: <https://docs.google.com/spreadsheets/d/1G9kDheY3MdITZnrthTFIUyprrTEvlMYpkFYWbCAMCmA/>

It creates a new sheet called `table-descriptions <date>` and populates it with the data from the CSV file.
...
1. Refresh the spreadsheet (F5)
    1. There should now be a "CSV Tools" menu with an item "Upload CSV → New Sheet" to run the script
1. Run the script
    1. Click "Upload CSV → New Sheet"
    1. Select the CSV file (output from the `export-table-descriptions.sh` script)
    1. Click "Import"
1. Check the results
    1. The new sheet should be created and populated with the data from the CSV file
