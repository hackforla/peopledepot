...
class Project(AbstractBaseModel):
    ...
    github_org_id = models.CharField(
        max_length=8,
        blank=True,
        help_text='Can be retrieved from gh api with the following: curl -H "Authorization: token [gh_PAT]" https://api.github.com/orgs/[org]',
    )
    github_primary_repo_id = models.CharField(
        max_length=9,
        blank=True,
        help_text='Can be retrieved from gh api with the following: curl -H "Authorization: token [gh_PAT]" https://api.github.com/repos/[org]/[repo]',
    )
