...
from core.models import (
    Affiliate, Affiliation, CheckType, Event, EventType, Faq, FaqViewed,
    LeadershipType, Location, PermissionType, PracticeArea, ProgramArea,
    Project, ProjectStatus, Referrer, ReferrerType, Sdg, Skill, SocMajor,
    StackElement, StackElementType, UrlType, User, UserPermission, UserStatusType,
)
...
class BaseUserSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        ...
        fields = (
            ...
            "is_superuser", "is_active", "is_staff", "email",
            "first_name", "last_name", "email_gmail", "email_preferred",
            "job_title_current_intake", "job_title_target_intake",
            "current_skills", "target_skills", "referrer", "linkedin_account",
            "github_handle", "slack_id", "phone", "texting_ok",
            "time_zone", "practice_area_primary", "practice_area_secondary",
            "practice_area_target_intake", "email_cognito", "user_status",
        )
...
class UserPermissionSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        ...
        fields = ("uuid", "created_at", "updated_at", "user", "permission_type", "project", "practice_area")
...
class ProjectSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        ...
        fields = (
            ...
            "completed_at", "github_org_id", "github_primary_repo_id",
            "hide", "leadership_type", "google_drive_id",
            "image_logo", "image_hero", "image_icon",
            "sdgs", "program_areas",
        )
...
class EventSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = Event
        fields = ("uuid", "name", "start_time", "duration_in_min", "video_conference_url", "additional_info", "project")
...
class AffiliateSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = Affiliate
        fields = ("uuid", "partner_name", "partner_logo", "is_active", "url", "is_org_sponsor", "is_org_partner")
...
class FaqSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        ...
        read_only_fields = ReadOnlyBaseSerializer.Meta.read_only_fields + ("created_on", "last_updated")
...
class LocationSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = Location
        fields = ("uuid", "name", "address_line_1", "address_line_2", "city", "state", "zip", "phone")
...
class StackElementSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = StackElement
        fields = ("uuid", "name", "description", "url", "logo", "active", "element_type")
...
class AffiliationSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = Affiliation
        fields = ("uuid", "affiliate", "project", "created_at", "ended_at", "is_sponsor", "is_partner")
...
class ReferrerSerializer(ReadOnlyBaseSerializer):
    ...
    class Meta(ReadOnlyBaseSerializer.Meta):
        model = Referrer
        fields = ("uuid", "name", "url", "referrer_type", "contact_name", "contact_email")
