...
class AccessControl:
    ...
    @classmethod
    def get_valid_post_request_fields(cls, request, table_name):
        ...
        fields = cls.get_permitted_fields(
            operation="post",
            table_name=table_name,
            permission_type=GLOBAL_ADMIN,
        )
        return fields

    @classmethod
    def get_valid_patch_request_fields(cls, request, table_name, response_related_user):
        requesting_user = request.user
        requesting_user = request.user
